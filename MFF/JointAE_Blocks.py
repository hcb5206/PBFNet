...
class JointAE_Blocks(nn.Module):
    def __init__(self, input_size, hidden_size, AE_num_layers, AE_num_blocks, pool_size, modal_sel, weigh=0.5):
        super(JointAE_Blocks, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.AE_num_layers = AE_num_layers
        self.AE_num_blocks = AE_num_blocks
        self.pool_size = pool_size
        self.modal_sel = modal_sel
        self.weigh = weigh

        self.encoder_A_1 = nn.Sequential(
            DenseBlock_Encoder(input_channels=input_size, num_layers=AE_num_layers, growth_rate=hidden_size),
            nn.Conv2d(in_channels=input_size + AE_num_layers * hidden_size, out_channels=hidden_size,
                      kernel_size=1)
        )
        self.encoder_A = nn.ModuleList([
            nn.Sequential(
                DenseBlock_Encoder(input_channels=hidden_size, num_layers=AE_num_layers, growth_rate=hidden_size),
                nn.Conv2d(in_channels=(AE_num_layers + 1) * hidden_size, out_channels=hidden_size,
                          kernel_size=1)
            )
            for _ in range(AE_num_blocks - 1)
        ])

        self.encoder_B_1 = nn.Sequential(
            DenseBlock_Encoder(input_channels=input_size, num_layers=AE_num_layers, growth_rate=hidden_size),
            nn.Conv2d(in_channels=input_size + AE_num_layers * hidden_size, out_channels=hidden_size,
                      kernel_size=1)
        )
        ...
        self.Reparameterize = ReparameterizeLayer(input_size=hidden_size * 2, pool_size=pool_size,
                                                  hidden_size=hidden_size)

        self.decoder = nn.ModuleList([
            nn.Sequential(
                DenseBlock_Decoder(input_channels=hidden_size, num_layers=AE_num_layers, growth_rate=hidden_size),
                nn.Conv2d(in_channels=(AE_num_layers + 1) * hidden_size, out_channels=hidden_size,
                          kernel_size=1)
            )
            for _ in range(AE_num_blocks)
        ])

        self.output = nn.Conv2d(in_channels=hidden_size, out_channels=input_size, kernel_size=1)

    def forward(self, x_A, x_B):
        encoded_A = self.encoder_A_1(x_A)
        encoded_B = self.encoder_B_1(x_B)
        for encoder_A_block, encoder_B_block in zip(self.encoder_A, self.encoder_B):
            encoded_A = encoder_A_block(encoded_A)
            encoded_B = encoder_B_block(encoded_B)

        combined = torch.cat((encoded_A, encoded_B), dim=1)

        simi = self.Reparameterize(combined)

        decoded = simi
        for decoder_block in self.decoder:
            decoded = decoder_block(decoded)
        if self.modal_sel == 'xA':
            return self.output(decoded) + x_A
        elif self.modal_sel == 'xB':
            return self.output(decoded) + x_B
        elif self.modal_sel == 'xAB':
            return self.output(decoded) + (self.weigh * x_A + (1 - self.weigh) * x_B)
        elif self.modal_sel == 'x_add':
            return self.output(decoded) + (x_A + x_B)
        elif self.modal_sel == 'x_max':
            return self.output(decoded) + torch.maximum(x_A, x_B)
        elif self.modal_sel == 'x_wav':
            return self.output(decoded) + wavelet_fusion(x_A, x_B, fusion_method='average').to(x_A.device)
        elif self.modal_sel == 'x_lap':
            return self.output(decoded) + laplacian_pyramid_fusion(x_A, x_B).to(x_A.device)
        elif self.modal_sel == 'x_nsct':
            return self.output(decoded) + nsct_fusion(x_A, x_B).to(x_A.device)
        elif self.modal_sel == 'x_ey':
            return self.output(decoded) + energy_minimization_fusion(x_A, x_B).to(x_A.device)
        elif self.modal_sel == 'x_gra':
            return self.output(decoded) + gradient_field_fusion(x_A, x_B).to(x_A.device)
        elif self.modal_sel == 'x_fea':
            return self.output(decoded) + feature_level_fusion(x_A, x_B).to(x_A.device)
        elif self.modal_sel == 'x_lr':
            return self.output(decoded) + low_rank_matrix_decomposition_fusion(x_A, x_B).to(x_A.device)
        elif self.modal_sel == 'x_gui':
            return self.output(decoded) + guided_filter_fusion(x_A, x_B).to(x_A.device)
