...
from metrics_Lytro import ssim, VIF, EN, SF, q_abf, AG
...
def model_test(model, dataloader_test, device):
    ...
    with torch.no_grad():
        for batch_idx, (near, far, _) in enumerate(dataloader_test):
            ...
            flops, _ = profile(model, inputs=(near, far))
            all_flops += flops / 1e9
...
def model_test_visualization(model_path, dataset_test, modal_sel):
    ...
    model.eval()
    with torch.no_grad():
        for batch_idx, (near_focus, far_focus, _) in enumerate(dataloader_test):
...
parser.add_argument("--model_path", type=str, default='model/JointAE_Sec_B_ssim_1',
                    help="")
opt = parser.parse_args()
print(opt)

dataset, _, dataset_test = dataloader()
