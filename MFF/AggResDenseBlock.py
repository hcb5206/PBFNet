...
class DenseBlock_Encoder(nn.Module):
    def __init__(self, input_channels, num_layers, growth_rate):
        ...
        self.out_res = nn.Conv2d(input_channels + num_layers * growth_rate, input_channels, kernel_size=1)
        self.aggregated = nn.Sequential(
            nn.Conv2d(in_channels=input_channels, out_channels=growth_rate, kernel_size=1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(in_channels=growth_rate, out_channels=growth_rate, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(in_channels=growth_rate, out_channels=input_channels, kernel_size=1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
    ...
    def forward(self, x):
        x_dense = x
        features_cat = [x_dense]
        for layer in self.layers:
            new_feature_out = layer(x_dense)
            features_cat.append(new_feature_out)
            x_dense = torch.cat(features_cat, dim=1)
        return self.out_res(torch.cat(features_cat, dim=1)) + self.aggregated(x) + x
...
class JointAE(nn.Module):
    def __init__(self, input_size, hidden_size, AE_num_layers, pool_size, modal_sel):
        ...
        self.encoder_B = DenseBlock_Encoder(input_channels=input_size, num_layers=AE_num_layers,
                                            growth_rate=hidden_size)
        self.Reparameterize = ReparameterizeLayer(input_size=input_size * 2, pool_size=pool_size,
