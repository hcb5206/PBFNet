...
def wavelet_fusion(img1, img2, wavelet='db1', fusion_method='average'):
    ...
    for b in range(batch_size):
        for c in range(channels):
            ...
            fused_channel_resized = fused_channel[:height, :width]
            if fused_channel_resized.shape != (height, width):
                fused_channel_resized = np.pad(
                    fused_channel_resized,
                    ((0, height - fused_channel_resized.shape[0]), (0, width - fused_channel_resized.shape[1])),
                    mode='constant'
                )

            fused_imgs[b, c] = torch.tensor(fused_channel_resized, dtype=img1.dtype, device=img1.device)
