...
def model_test(model, dataloader_test, device):
    ...
    input_ag = 0.0
    with torch.no_grad():
        for batch_idx, (near, far, _) in enumerate(dataloader_test):
            near = near.to(device)
            far = far.to(device)
...
def model_test_visualization(dataset_test):
    ...
    with torch.no_grad():
        for batch_idx, (near_focus, far_focus, labels) in enumerate(dataloader_test):
            ...
            # save_and_visualize_image(output_np, 'image_res', img_idx=batch_idx + 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_epochs", type=int, default=10000, help="")
    parser.add_argument("--batch_size", type=int, default=2, help="")
    parser.add_argument("--loss", type=str, default='MSE', help="MSE or MAE or CIL")
    parser.add_argument("--if_blocks", type=str, default='yes', help="yes or no")
    parser.add_argument("--input_size", type=int, default=3, help="")
    parser.add_argument("--hidden_size", type=int, default=64, help="")
    parser.add_argument("--AE_num_layers", type=int, default=2, help="")
    parser.add_argument("--AE_num_blocks", type=int, default=3, help="")
    ...
    parser.add_argument("--model_path", type=str, default='model/JointAE_blocks_3',
                        help="")
    opt = parser.parse_args()

    _, dataset_test, _ = dataloader()
