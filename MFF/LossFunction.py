...
class GradientLoss(nn.Module):
    def __init__(self, threshold, gamma, operator='sobel'):
        ...
        self.kernels = {
            'laplacian': torch.tensor([[0, 1, 0],
                                       [1, -4, 1],
                                       [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0).repeat(3, 1, 1,
                                                                                                         1).cuda(),
            'sobel_x': torch.tensor([[-1, 0, 1],
                                     [-2, 0, 2],
                                     [-1, 0, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0).repeat(3, 1, 1,
                                                                                                        1).cuda(),
            'sobel_y': torch.tensor([[1, 2, 1],
                                     [0, 0, 0],
                                     [-1, -2, -1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0).repeat(3, 1, 1,
                                                                                                          1).cuda(),
            ...
            'prewitt_y': torch.tensor([[1, 1, 1],
                                       [0, 0, 0],
                                       [-1, -1, -1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0).repeat(3, 1, 1,
                                                                                                            1).cuda()
        }
    ...
    def forward(self, pred, target):
        if self.operator == 'sobel':
            gradient_pred_x = F.conv2d(pred, self.kernels['sobel_x'], padding=1, groups=3)
            gradient_pred_y = F.conv2d(pred, self.kernels['sobel_y'], padding=1, groups=3)
            gradient_target_x = F.conv2d(target, self.kernels['sobel_x'], padding=1, groups=3)
            gradient_target_y = F.conv2d(target, self.kernels['sobel_y'], padding=1, groups=3)

            gradient_pred = torch.sqrt(gradient_pred_x ** 2 + gradient_pred_y ** 2 + 1e-10)
            gradient_target = torch.sqrt(gradient_target_x ** 2 + gradient_target_y ** 2 + 1e-10)

        elif self.operator == 'prewitt':
            gradient_pred_x = F.conv2d(pred, self.kernels['prewitt_x'], padding=1, groups=3)
            gradient_pred_y = F.conv2d(pred, self.kernels['prewitt_y'], padding=1, groups=3)
            gradient_target_x = F.conv2d(target, self.kernels['prewitt_x'], padding=1, groups=3)
            gradient_target_y = F.conv2d(target, self.kernels['prewitt_y'], padding=1, groups=3)

            gradient_pred = torch.sqrt(gradient_pred_x ** 2 + gradient_pred_y ** 2 + 1e-10)
            gradient_target = torch.sqrt(gradient_target_x ** 2 + gradient_target_y ** 2 + 1e-10)

        else:
            gradient_pred = F.conv2d(pred, self.kernels['laplacian'], padding=1, groups=3)
            gradient_target = F.conv2d(target, self.kernels['laplacian'], padding=1, groups=3)

        gradient_pred = torch.abs(gradient_pred)
        gradient_target = torch.abs(gradient_target)

        gradient_pred = (gradient_pred - gradient_pred.min()) / (
                gradient_pred.max() - gradient_pred.min() + 1e-10)
        gradient_target = (gradient_target - gradient_target.min()) / (
                gradient_target.max() - gradient_target.min() + 1e-10)

        gradient_pred_thresh = gradient_pred.clone()
        gradient_target_thresh = gradient_target.clone()

        gradient_pred_thresh[gradient_pred_thresh < self.threshold] = 0
        gradient_target_thresh[gradient_target_thresh < self.threshold] = 0

        gradient_pred = self.gamma_correction(gradient_pred_thresh, self.gamma)
        gradient_target = self.gamma_correction(gradient_target_thresh, self.gamma)

        loss = F.l1_loss(gradient_pred, gradient_target)
...
class M3Loss(nn.Module):
    def __init__(self, fs, loss, threshold, gamma, l_int, l_ncc, l_ssim, l_grad):
        ...
        self.gradient = GradientLoss(threshold=threshold, gamma=gamma)

    def forward(self, output, label):
        if self.loss == 'MSE':
            loss_fusion = self.l_int * self.mse(output, label)
        elif self.loss == 'MAE':
            loss_fusion = self.l_int * self.mae(output, label)
        else:
            loss_fusion = self.l_int * self.cil(output, label)
        loss_ncc = self.l_ncc * self.ncc(output, label)
        loss_ssim = self.l_ssim * self.ssim(output, label)
        loss_grad = self.l_grad * self.gradient(output, label)
        # print(loss_fusion)
        # print(loss_ncc)
        # print(loss_ssim)
        # print(loss_grad)
        loss_output = loss_fusion + loss_ncc + loss_ssim + loss_grad
        # loss_output = loss_grad
        return loss_output
