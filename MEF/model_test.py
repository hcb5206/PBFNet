...
from metrics_all import rmse, mse, psnr, ssim, VIF, ms_ssim, SF, SD, AG
from math import exp


def save_output_image(output, save_dir, img_idx):
...
def model_test(model, dataloader_test, device):
    ...
    with torch.no_grad():
        for batch_idx, (underexposed, overexposed, labels) in enumerate(dataloader_test):
            ...
            input_rmse += rmse(overexposed, labels)
            input_psnr += psnr(overexposed, labels)
            input_ssim += ssim(overexposed, labels)
            input_vif += VIF(overexposed, labels)
            input_sf += SF(overexposed)
            input_sd += SD(overexposed)
            input_ag += AG(overexposed)
...
def model_test_visualization(dataset_test):
    ...
    with torch.no_grad():
        for batch_idx, (underexposed, overexposed, labels) in enumerate(dataloader_test):
            ...
            # overexposed_np = overexposed.cpu().numpy().squeeze()
            # underexposed_np = underexposed.cpu().numpy().squeeze()
            # labels_np = labels.cpu().numpy().squeeze()
            output_np = output.cpu().numpy().squeeze()

            # overexposed_np = overexposed_np.transpose(1, 2, 0)
            # underexposed_np = underexposed_np.transpose(1, 2, 0)
            ...
            #
            # axes[0, 0].imshow(underexposed_np)
            # axes[0, 0].set_title('Near Focus')
            # axes[0, 0].axis('off')
            #
            # axes[0, 1].imshow(overexposed_np)
