...
class DenseLayer_Encoder(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.DenseLayer = nn.Sequential(
            ...
            nn.InstanceNorm2d(num_features=hidden_size, eps=1e-8, affine=True),
        )
...
class DenseBlock_Encoder(nn.Module):
    def __init__(self, input_channels, num_layers, hidden_size):
        super(DenseBlock_Encoder, self).__init__()
        self.conv_res = nn.Conv2d(input_channels, hidden_size, kernel_size=1)
        self.layers = nn.ModuleList()
        for i in range(num_layers):
            layer = DenseLayer_Encoder(hidden_size)
...
class ReparameterizeLayer(nn.Module):
    def __init__(self, pool_size, hidden_size):
        ...
        self.w2 = nn.Conv2d(in_channels=hidden_size * 2, out_channels=hidden_size, kernel_size=1)
...
class JointAE(nn.Module):
    def __init__(self, input_size, hidden_size, AE_num_layers, pool_size, modal_sel):
        ...
        self.encoder_B = DenseBlock_Encoder(input_channels=input_size, num_layers=AE_num_layers,
                                            hidden_size=hidden_size)
        self.Reparameterize = ReparameterizeLayer(pool_size=pool_size, hidden_size=hidden_size)
    ...
    def forward(self, x_A, x_B):
        ...
        if self.modal_sel == 'xA':
            ...
            return self.output(self.decoder(simi)) + (x_A + x_B) / 2
...
#     torch.manual_seed(1)
#     x1 = torch.rand(2, 3, 424, 624, dtype=torch.float32).to(device)
#     x2 = torch.rand(2, 3, 424, 624, dtype=torch.float32).to(device)
