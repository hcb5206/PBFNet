...
class M3Loss(nn.Module):
    def __init__(self, fs, loss, threshold, gamma):
    ...
    def forward(self, output, label):
        if self.loss == 'MSE':
            loss_fusion = self.mse(output, label)
        elif self.loss == 'MAE':
            loss_fusion = self.mae(output, label)
        else:
            loss_fusion = self.cil(output, label)
        loss_output = loss_fusion + self.ncc(output, label) + \
                      0.1 * self.ssim(output, label) + self.gradient(output, label)
        # loss_output = self.ssim(output, label)
        return loss_output
