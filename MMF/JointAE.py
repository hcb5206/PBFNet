...
class JointAE(nn.Module):
    ...
    def forward(self, x_A, x_B):
        ...
        if self.modal_sel == 'xA':
            ...
            return self.output(self.decoder(simi)) + torch.clip(torch.add(x_A, x_B), 0, 1)
        elif self.modal_sel == 'x_max':
            return self.output(self.decoder(simi)) + torch.maximum(x_A, x_B)
        elif self.modal_sel == 'x_wav':
            return self.output(self.decoder(simi)) + wavelet_fusion(x_A, x_B, fusion_method='max').to(x_A.device)
