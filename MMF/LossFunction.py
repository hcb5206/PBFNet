...
class GradientLoss(nn.Module):
    ...
    def forward(self, pred, vi, ir):
        if self.operator == 'sobel':
            ...
            gradient_vi_x = F.conv2d(vi, self.kernels['sobel_x'], padding=1, groups=3)
            gradient_vi_y = F.conv2d(vi, self.kernels['sobel_y'], padding=1, groups=3)
            gradient_ir_x = F.conv2d(ir, self.kernels['sobel_x'], padding=1, groups=3)
            gradient_ir_y = F.conv2d(ir, self.kernels['sobel_y'], padding=1, groups=3)
            ...
            gradient_vi_x = F.conv2d(vi, self.kernels['prewitt_x'], padding=1, groups=3)
            gradient_vi_y = F.conv2d(vi, self.kernels['prewitt_y'], padding=1, groups=3)
            gradient_ir_x = F.conv2d(ir, self.kernels['prewitt_x'], padding=1, groups=3)
            gradient_ir_y = F.conv2d(ir, self.kernels['prewitt_y'], padding=1, groups=3)
            ...
            gradient_vi = F.conv2d(vi, self.kernels['laplacian'], padding=1, groups=3)
            gradient_ir = F.conv2d(ir, self.kernels['laplacian'], padding=1, groups=3)
    ...
    def forward(self, output, vi, ir):
        ...
        if self.loss == 'MSE':
            if self.int_style == 'max':
                ...
                loss_fusion = 0.5 * self.mse(output, torch.maximum(vi, ir)) + \
                              0.5 * self.mse(output, torch.maximum(mask * vi, ir))
            ...
            if self.int_style == 'max':
                ...
                loss_fusion = 0.5 * self.mae(output, torch.maximum(vi, ir)) + \
                              0.5 * self.mae(output, torch.maximum(mask * vi, ir))
            elif self.int_style == 'mask_g':
            ...
            if self.int_style == 'max':
                ...
                loss_fusion = 0.5 * self.cil(output, torch.maximum(vi, ir)) + \
                              0.5 * self.cil(output, torch.maximum(mask * vi, ir))
            elif self.int_style == 'mask_g':
