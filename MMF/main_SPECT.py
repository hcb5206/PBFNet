...
from LossFunction_SPECT import M3Loss
...
def train():
    ...
    parser.add_argument("--tasks", type=str, default='SPECT', help="PET or SPECT")
    parser.add_argument("--model_path", type=str, default='model_train/SPECT/model.pth',
                        help="")
    ...
    for epoch in range(opt.n_epochs):
        ...
        for batch_idx, (mri, pet) in enumerate(dataloader_train):
            ...
            loss = criterion(output_y, mri_y, pet_y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        folder_path = os.path.join("model_train/SPECT", f"{opt.model_sel}_{opt.tasks}_{opt.prior_sel}_{opt.int_style}")
