...
def train():
    ...
    parser.add_argument("--test_dataset", type=str, default='MSRS', help="LLVIP or M3FD or MSRS")
    ...
    dataset, dataset_test = dataloader(test_dataset=opt.test_dataset)
    ...
    for epoch in range(opt.n_epochs):
        ...
        for batch_idx, (vi, ir) in enumerate(dataloader_train):
            vi = vi.to(device)
            ir = ir.to(device)

            output = model(vi, ir)
            loss = criterion(output, vi, ir)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            train_vif += VIF(output, vi, ir)
            train_en += EN(output)
            train_ssim += ssim(output, vi, ir)
            train_sf += SF(output)
            train_q_abf += q_abf(output, vi, ir)
            train_ag += AG(output)
        ...
        folder_path = os.path.join("model_train", f"{opt.if_th}_{opt.prior_sel}_{opt.int_style}")
