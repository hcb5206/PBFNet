...
def model_test(model, dataloader_test, device):
    ...
    # all_time = 0.0
    with torch.no_grad():
        for batch_idx, (vi, ir) in enumerate(dataloader_test):
            vi = vi.to(device)
            ir = ir.to(device)
            ...
            input_img = torch.maximum(vi, ir)

            input_vif += VIF(input_img, vi, ir)
            input_en += EN(input_img)
            input_ssim += ssim(input_img, vi, ir)
            input_sf += SF(input_img)
            input_q_abf += q_abf(input_img, vi, ir)
            input_ag += AG(input_img)

            test_vif += VIF(output, vi, ir)
            test_en += EN(output)
            test_ssim += ssim(output, vi, ir)
            test_sf += SF(output)
            test_q_abf += q_abf(output, vi, ir)
            test_ag += AG(output)
...
def model_test_visualization(dataset_test):
    ...
    # print(torch.load(opt.model_path).keys())
    ...
    with torch.no_grad():
        for batch_idx, (vi, ir) in enumerate(dataloader_test):
            ...
            # vi = normalize_image(vi)
            # ir = normalize_image(ir)
...
parser.add_argument("--test_dataset", type=str, default='MSRS', help="LLVIP or M3FD or MSRS or all or 0037")
parser.add_argument("--int_style", type=str, default='max', help="max or or mean or mask_n or mask_g or no_ir")
...
# output_csv_path = f"./results_csv/{opt.test_dataset}.csv"
#
# if not os.path.exists(output_csv_path):
#     with open(output_csv_path, mode='w', newline='') as file:
#         writer = csv.writer(file)
#         writer.writerow(["Dataset", opt.test_dataset])
#         writer.writerow(["Sample Index", "VIF", "EN", "SSIM", "SF", "Q_ABF", "AG"])

_, dataset_test = dataloader(opt.test_dataset)
