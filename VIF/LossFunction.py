...
def generate_mask(ir, mask_th) -> torch.Tensor:
    mask = (ir >= mask_th).float()
    mask = RGBYCrCb(mask)
    return mask
...
class M3Loss(nn.Module):
    ...
    def forward(self, output, vi, ir):
        mask = generate_mask(ir, self.mask_th)
        if self.loss == 'MSE':
            if self.int_style == 'max':
                ...
                loss_fusion = self.mse(output, torch.maximum(vi, mask * ir))
            ...
            if self.int_style == 'max':
                ...
                loss_fusion = self.mae(output, torch.maximum(vi, mask * ir))
            elif self.int_style == 'mask_g':
            ...
            if self.int_style == 'max':
                ...
                loss_fusion = self.cil(output, torch.maximum(vi, mask * ir))
            elif self.int_style == 'mask_g':
